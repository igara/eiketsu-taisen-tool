...
export const PowerDetails: React.FC<Props> = ({ formMethod }) => {
	const { powers, refDetailsElement, onKeyDownSummary, onClickWrapper } =
		useLogic();
			...
			<div className="absolute z-10 bottom-[28px] w-[60px] flex flex-col gap-1 p-1 bg-[#252423] rounded-[4px]">
				{powers.map((power) => (
					...
					>
						...
						/>
						<label htmlFor={`power_${power}`} className="text-black">
							{power}
						</label>
					</div>
				))}
